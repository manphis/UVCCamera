...
public class AVIGenerator {
    ...
    public AVIGenerator(File aviFile, int width, int height, double framerate, int numFrames) throws Exception {
    ...
    public void writeImage(ByteBuffer dataBuffer) throws Exception {
        byte[] fcc = new byte[] { '0', '0', 'd', 'b' };
//        byte[] imagedata = writeImageToBytes(image);
        dataBuffer.position(0);
        byte[] imagedata = new byte[dataBuffer.capacity()];
        dataBuffer.get(imagedata, 0, imagedata.length);
